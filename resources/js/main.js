(function ($) {
  ...
  $(document).ready(function () {
    ...
    if ($screenshotCarousel.length > 0) {
      $screenshotCarousel.owlCarousel({
        ...
        smartSpeed: 1000,
        navText: [
          '<img src="assets/img/startup/left.svg" alt="arrow">',
          '<img src="assets/img/startup/right.svg" alt="arrow">',
        ],
        responsive: {
          0: {
            items: 1,
    ...
    $(document).on("click", "#body-overlay", function (e) {
      e.preventDefault();
      clientPopup.removeClass("active");
      bodyOvrelay.removeClass("active");
    });
    $(document).on("click", ".client-btn", function (e) {
      e.preventDefault();
      clientPopup.addClass("active");
      bodyOvrelay.addClass("active");
    });
     ...
     *  file upload
     * -------------------------------------*/
    if ($(".MapUI-file-input").length) {
      $(".MapUI-file-input").on("change", function () {
  ...
  /* ------- Gallery image Popup--------- */
  MapUI_image_popup(".gallery-masonry .masonry-item");
  ...
  //       sourcesSelector[i].querySelector('option[value^="rtmp"]').disabled =
  //         true;
  //     }
  //     if (sourcesSelector[i].querySelector('option[value$="webm"]')) {
  //       sourcesSelector[i].querySelector('option[value$="webm"]').disabled =
  //         true;
  //     }
  //     if (sourcesSelector[i].querySelector('option[value$=".mpd"]')) {
  //       sourcesSelector[i].querySelector('option[value$=".mpd"]').disabled =
  //         true;
  //     }
  //     if (sourcesSelector[i].querySelector('option[value$=".ogg"]')) {
  //       sourcesSelector[i].querySelector('option[value$=".ogg"]').disabled =
  //         true;
  //     }
  //     if (sourcesSelector[i].querySelector('option[value$=".flv"]')) {
  //       sourcesSelector[i].querySelector('option[value*=".flv"]').disabled =
  //         true;
  //     }
  //   }
  // }
  ...
  // code for footer news images date
  $(document).ready(function () {
    ...
    $(".media-body span").each(function () {
      $(this).text(formatDate(today));
    });
  });
  ...
  if (window.innerWidth < 1024) {
    document.querySelectorAll(".mega-menu-item").forEach((item) => {
      item.addEventListener("click", (e) => {
        ...
        document
          .querySelectorAll(".mega-menu-item.is-open")
          .forEach((other) => {
    ...
    document.addEventListener("click", (e) => {
      if (!e.target.closest(".mega-menu-item")) {
        document.querySelectorAll(".mega-menu-item.is-open").forEach((item) => {
...
document.querySelectorAll(".subcribe-form").forEach((form) => {
    ...
    fetch("https://mapui1.aibuzz.net/send_mail", {
      ...
      .then((data) => {
        ...
        if (data.result === "success") {
          ...
          this.reset();
        } else if (data.result === "error" && data.status === 400) {
...
(function () {
  var img = document.querySelector(".computer-img");
  if (!img) return;
  ...
  function animateIn() {
    ...
    // Only trigger when the FULL image top is within viewport
  ...
  // ✅ Only runs on actual scroll — never on page load
  window.addEventListener("scroll", animateIn, { passive: true });
})();
...
$(function () {
  ...
  // ── Close on × button ─────────────────────────────────────
  $(document).on("click", "#sales-enquiry-close", function () {
    closePopup();
  });

  // ── Close on outside click (overlay itself, not modal) ────
  $(document).on("click", "#sales-enquiry-overlay", function (e) {
    if ($(e.target).is("#sales-enquiry-overlay")) {
      closePopup();
    }
  });
